...
//                     this.y_vel = 2*normal_y;                    
        ...
        var temp1 = 0;
        var temp2 = 0;
        if (this.left_ctrl && this.x_vel > -10) temp1 += -10 * this.left_ctrl * delta_time;
        if (this.right_ctrl && this.x_vel < 10) temp1 += 10 * this.right_ctrl * delta_time;
        if (this.up_ctrl && this.y_vel < 10)    temp2 += 10 * this.up_ctrl * delta_time;
        if (this.down_ctrl && this.y_vel > -10) temp2 += -10 * this.down_ctrl * delta_time;
        if (temp1 == 0 && temp2 == 0) {
            temp1 /= Math.sqrt(1/2);
            temp2 /= Math.sqrt(1/2);
        }
        this.x_vel += temp1 + this.x_acc * delta_time;
        this.y_vel += temp2 + this.y_acc * delta_time;
        ...
        this.x_rot += this.x_vel * delta_time / 2;
        this.y_rot += this.y_vel * delta_time / 2;
        
        this.theta = Math.atan2(this.y_vel, this.x_vel);
...
//        Draw the four walls of the baseboard (remains in all levels)
        ...
        this.shapes.ball.draw(graphics_state, 
                Mat4.identity()
                    ...
                    .times(Mat4.rotation(Math.sqrt(Math.pow(this.x_coord, 2) + Math.pow(this.y_coord, 2)) / 2, Vec.of(0, 1, 0)))
                    .times(Mat4.rotation(this.theta, Vec.of(0, 0, 1))),
                    // .times(Mat4.rotation(this.x_rot, Vec.of(0, 1, 0)));
                    // .times(Mat4.rotation(this.y_rot, Vec.of(1, 0, 0)));
                    this.shape_materials.ball || this.plastic.override({color: this.lightgrey}));
        
        if(this.t % 100)
            //console.log(this.x_vel, this.x_rot);
